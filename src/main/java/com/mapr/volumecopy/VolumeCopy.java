 ...
 */
public class VolumeCopy implements Tool {

    public static final Log LOG = LogFactory.getLog(VolumeCopy.class);

    private static final String NAME = "volumecopy";

    private static MapRFileSystem fs;

    private static ArrayList<FileStatus> afs= new  ArrayList<FileStatus>();
    ...
    static enum Options {
        ...
        FILE_SIZE_MIN("-filesizemin", NAME + ".file.size.min"),
        FILE_SIZE_MAX("-filesizemax", NAME + ".file.size.max");
    ...
    public VolumeCopy(Configuration conf) {
        setConf(conf);
    }
    ...
    static class CopyFilesMapper
        ...
        private void copy(FileStatus srcstat, Path relativedst,
            ...
            try {
                ...
                try {
                    ...
                    // Bug 12722: To help figure out why NPE is thrown
                    LOG.error("Hit NPE for: " + srcstat.getPath());
                    throw e;
                }
                ...
                try {
                    if (in != null) {
                        in.adviseFile(FadviseType.FILE_DONTNEED, 0, cbcopied);
                    }
        ...
        private void updatePermissions(FileStatus src, FileStatus dst
            ...
            if (preserve_status) {
                VolumeCopy.updatePermissions(src, dst, preseved, destFileSys);
            }
        }
        ...
        public void map(LongWritable key,
            ...
            try {
                ...
                if(e.getMessage().contains("Not a directory(20)") || e.getMessage().contains("Open failed for file")){
    ...
    public static void copy(Configuration conf, String srcPath,
        ...
        copy(conf, new Arguments(tmp, dst, logPath, flags, null,
                Long.MAX_VALUE, Long.MAX_VALUE, 0, Long.MAX_VALUE, null));
    }
    ...
    static private class Arguments {
        ...
        Arguments(List<Path> srcs, Path dst, Path log,
                  ...
                  long filelimit, long sizelimit, long filesizemin, long filesizemax,
        ...
        static Arguments valueOf(String[] args, Configuration conf
            ...
            for (int idx = 0; idx < args.length; idx++) {
                ...
                if (i < opt.length) {
                    ...
                    else if (opt[i] == Options.FILE_SIZE_MIN) {
                        filesizemin = Options.FILE_SIZE_MIN.parseLong(args, ++idx);
                    }
                    else if (opt[i] == Options.FILE_SIZE_MAX) {
            ...
            if (filesizemin > filesizemax) {
                throw new IllegalArgumentException(Options.FILE_SIZE_MAX.cmd
                        + " must be greater than or equal to " + Options.FILE_SIZE_MIN.cmd + ".");
            }
            ...
            return new Arguments(srcs, dst, log, flags, presevedAttributes,
                    filelimit, sizelimit, filesizemin, filesizemax, mapredSslConf);
        }
    ...
    public static MapRFileSystem initializeMapRFileSystem() {
        ...
        try {
            ...
            fs.initialize(URI.create("maprfs:///"), new Configuration(), true);
    ...
    public static ArrayList<FileStatus> getDirectoryStructure(MapRFileSystem fs, Path srcPath){

        try {
            FileStatus[] fileStatus = fs.listStatus(srcPath);
            for(FileStatus f:fileStatus){
                System.out.println("getDirectoryStructure :: "+f.getPath().getName());
                System.out.println("getDirectoryStructure :: "+f.getPath().toUri().getPath());
                if(f.isDirectory()){
                    getDirectoryStructure(fs,f.getPath());
                }
                afs.add(f);
            }
        } catch (IOException e) {
    ...
    public static void main(String[] args) throws Exception {
        ...
        MapRHTable maprTable;
        FileSystem fileSystem = null;

        fs = initializeMapRFileSystem();
        ...
        for(FileStatus fileStatus:finalFileStatusList){
            ...
            if(fileSystem.getFileStatus(new Path(fileStatus.getPath().toUri().getPath())).isTable()){
        ...
        JobConf job = new JobConf(VolumeCopy.class);

        // Bug 12079: By default, we map the HDFS Impl to MapRFS to provide out-of-the-box
        // compatibility with Hadoop cluster.
        // But, this will cause problems while doing VolumeCopy across clusters, hence we correct
        // that setting here.
    ...
    private static JobConf createJobConf(Configuration conf) {
        JobConf jobconf = new JobConf(conf, VolumeCopy.class);
            ...
            for(Iterator<Path> srcItr = args.srcs.iterator(); srcItr.hasNext(); ) {
                ...
                for(pathstack.push(srcfilestat); !pathstack.empty(); ) {
                    ...
                    for(int i = 0; i < children.length; i++) {
                        ...
                        else if (child.getLen() < args.filesizemin
                                || child.getLen() >= args.filesizemax) {
                            //skip file if its size is not within requested range
                            skipfile = true;
                            LOG.info("Skipping file '" + child.getPath() +  "' as its size "
                                    + child.getLen() + " is outside of the reqested filesize range.");
                        }
                        else {
